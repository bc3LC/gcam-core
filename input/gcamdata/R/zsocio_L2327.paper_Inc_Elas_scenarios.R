# Copyright 2019 Battelle Memorial Institute; see the LICENSE file.

#' module_socio_L2327.paper_Inc_Elas_scenarios
#'
#' Calculates paper income elasticity for each GCAM region by linear interpolation of assumption data
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{L2327.paper_incelas_gcam3}, \code{object}. The corresponding file in the
#' original data system was \code{L2327.paper_Inc_Elas_scenarios.R} (socioeconomics level2).
#' @details Takes per-capita GDP from ssp scenarios in each region.
#' Then calculates paper income elasticity for each region by linear interpolation of assumption data.
#' @importFrom assertthat assert_that
#' @importFrom dplyr arrange filter left_join mutate select transmute
#' @importFrom tidyr gather spread
#' @importFrom stats approx
#' @author MMC July 2022
module_socio_L2327.paper_Inc_Elas_scenarios <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "common/GCAM_region_names",
             FILE = "socioeconomics/A327.inc_elas",
             "L102.pcgdp_thous90USD_Scen_R_Y",
             "L101.Pop_thous_GCAM3_R_Y",
             "L102.gdp_mil90usd_GCAM3_R_Y"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("L2327.paper_incelas_ssp1",
             "L2327.paper_incelas_ssp2",
             "L2327.paper_incelas_ssp3",
             "L2327.paper_incelas_ssp4",
             "L2327.paper_incelas_ssp5",
             "L2327.paper_incelas_gcam3"))
  } else if(command == driver.MAKE) {

    GCAM_region_ID <- value <- year <- pcgdp_90thousUSD <- scenario <-
      region <- energy.final.demand <- income.elasticity <- . <-
      value.x <- value.y <- NULL # silence package check.

    all_data <- list(...)[[1]]

    # Load required inputs
    GCAM_region_names <- get_data(all_data, "common/GCAM_region_names")
    A327.inc_elas <- get_data(all_data, "socioeconomics/A327.inc_elas", strip_attributes = TRUE)
    L102.pcgdp_thous90USD_Scen_R_Y <- get_data(all_data, "L102.pcgdp_thous90USD_Scen_R_Y", strip_attributes = TRUE)
    L101.Pop_thous_GCAM3_R_Y <- get_data(all_data, "L101.Pop_thous_GCAM3_R_Y", strip_attributes = TRUE)
    L102.gdp_mil90usd_GCAM3_R_Y <- get_data(all_data, "L102.gdp_mil90usd_GCAM3_R_Y", strip_attributes = TRUE)

    # ===================================================
    # Linearly interpolate income elasticity for each level of per-capita GDP,
    # using the assumption data
    L102.pcgdp_thous90USD_Scen_R_Y %>%
      rename(pcgdp_90thousUSD = value) %>%
      mutate(year = as.integer(year)) %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      filter(year %in% MODEL_FUTURE_YEARS) %>%
      # Using approx rather than approx_fun because data is from assumption file, not in our tibble
      mutate(income.elasticity = approx(x = filter(A327.inc_elas,A327.inc_elas$sector == 'paper')$pcgdp_90thousUSD,
                                        y = filter(A327.inc_elas,A327.inc_elas$sector == 'paper')$inc_elas,
                                        xout = pcgdp_90thousUSD,
                                        # Rule 2 means that data outside of the interval of input
                                        # data will be assigned the cloest data extreme
                                        rule = 2)[['y']] %>% round(3),
             energy.final.demand = "paper") %>%
      select(scenario, region, energy.final.demand, year, income.elasticity) %>%
      arrange(year) ->
      L2327.pcgdp_thous90USD_Scen_R_Y

    # CB: Make sure that Income Elasticity is identical in 2025 and diverges only in 2030
    # for MODEL_FIRST_FUTURE_YEAR, use SSP2 data for all 5 SSPs
    L2327.pcgdp_thous90USD_Scen_R_Y <- L2327.pcgdp_thous90USD_Scen_R_Y |> filter(
      year > MODEL_FIRST_FUTURE_YEAR) |> rbind(
        L2327.pcgdp_thous90USD_Scen_R_Y |> filter(year == MODEL_FIRST_FUTURE_YEAR, scenario == "SSP2")
      )|> rbind(L2327.pcgdp_thous90USD_Scen_R_Y |> filter(year == MODEL_FIRST_FUTURE_YEAR, scenario == "SSP2") |> mutate(scenario = "SSP1")
      )|> rbind(L2327.pcgdp_thous90USD_Scen_R_Y |> filter(year == MODEL_FIRST_FUTURE_YEAR, scenario == "SSP2") |> mutate(scenario = "SSP3")
      )|> rbind(L2327.pcgdp_thous90USD_Scen_R_Y |> filter(year == MODEL_FIRST_FUTURE_YEAR, scenario == "SSP2") |> mutate(scenario = "SSP4")
      )|> rbind(L2327.pcgdp_thous90USD_Scen_R_Y |> filter(year == MODEL_FIRST_FUTURE_YEAR, scenario == "SSP2") |> mutate(scenario = "SSP5")
      )

    # Split by scenario and remove scenario column from each tibble

    L2327.pcgdp_thous90USD_Scen_R_Y <- L2327.pcgdp_thous90USD_Scen_R_Y %>%
      split(.$scenario) %>%
      lapply(function(df) {select(df, -scenario) %>%
          add_units("Unitless (% change in service demand / % change in income)") %>%
          add_comments("Uses previously calculated per-capita GDP assumptions for all ssp scenarios") %>%
          add_comments("Paper income elasticity for each GCAM region generated by linear interpolation of assumption data") %>%
          add_precursors("common/GCAM_region_names", "socioeconomics/A327.inc_elas",
                         "L102.pcgdp_thous90USD_Scen_R_Y") })

    # L2327.paper_incelas_gcam3: paper sector income elasticity for GCAM 3.0 socioeconomics
    # For the GCAM 3.0 scenario, calculate the per-capita GDP
    L102.gdp_mil90usd_GCAM3_R_Y %>%
      left_join_error_no_match(L101.Pop_thous_GCAM3_R_Y, by = c("GCAM_region_ID", "year")) %>%
      filter(year %in% MODEL_FUTURE_YEARS) %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      transmute(region, year, pcgdp_90thousUSD = value.x / value.y) %>%
      # Using approx rather than approx_fun because data is from assumption file, not in our tibble
      mutate(income.elasticity = approx(x = filter(A327.inc_elas,A327.inc_elas$sector == 'paper')$pcgdp_90thousUSD,
                                        y = filter(A327.inc_elas,A327.inc_elas$sector == 'paper')$inc_elas,
                                        xout = pcgdp_90thousUSD,
                                        # Rule 2 means that data outside of the interval of input
                                        # data will be assigned the closest data extreme
                                        rule = 2)[['y']] %>% round(3),
             energy.final.demand = "paper") %>%
      select(region, energy.final.demand, year, income.elasticity) ->
      L2327.paper_incelas_gcam3


    # ===================================================

    # Produce outputs
    L2327.pcgdp_thous90USD_Scen_R_Y[["SSP1"]] %>%
      add_title("Paper Income Elasticity: ssp1") %>%
      add_legacy_name("L2327.paper_incelas_ssp1") ->
      L2327.paper_incelas_ssp1

    L2327.pcgdp_thous90USD_Scen_R_Y[["SSP2"]] %>%
      add_title("Paper Income Elasticity: ssp2") %>%
      add_legacy_name("L2327.paper_incelas_ssp2") ->
      L2327.paper_incelas_ssp2

    L2327.pcgdp_thous90USD_Scen_R_Y[["SSP3"]] %>%
      add_title("Paper Income Elasticity: ssp3") %>%
      add_legacy_name("L2327.paper_incelas_ssp3") ->
      L2327.paper_incelas_ssp3

    L2327.pcgdp_thous90USD_Scen_R_Y[["SSP4"]] %>%
      add_title("Paper Income Elasticity: ssp4") %>%
      add_legacy_name("L2327.paper_incelas_ssp4") ->
      L2327.paper_incelas_ssp4

    L2327.pcgdp_thous90USD_Scen_R_Y[["SSP5"]] %>%
      add_title("Paper Income Elasticity: ssp5") %>%
      add_legacy_name("L2327.paper_incelas_ssp5") ->
      L2327.paper_incelas_ssp5

    L2327.paper_incelas_gcam3 %>%
      add_title("paper Income Elasticity: gcam3") %>%
      add_units("Unitless (% change in service demand / % change in income)") %>%
      add_comments("Uses previously calculated per-capita GDP assumptions of paper elastciity") %>%
      add_comments("Paper income elasticity for each GCAM region generated by linear interpolation of assumption data") %>%
      add_legacy_name("L2327.paper_incelas_gcam3") %>%
      add_precursors("common/GCAM_region_names", "socioeconomics/A327.inc_elas",
                     "L101.Pop_thous_GCAM3_R_Y", "L102.gdp_mil90usd_GCAM3_R_Y") ->
      L2327.paper_incelas_gcam3

    return_data(L2327.paper_incelas_ssp1,
                L2327.paper_incelas_ssp2,
                L2327.paper_incelas_ssp3,
                L2327.paper_incelas_ssp4,
                L2327.paper_incelas_ssp5,
                L2327.paper_incelas_gcam3)
  } else {
    stop("Unknown command")
  }
}
