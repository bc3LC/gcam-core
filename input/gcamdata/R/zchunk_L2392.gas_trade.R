...
module_energy_L2392.gas_trade <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "common/GCAM_region_names",
             FILE = "energy/GCAM_region_pipeline_bloc", #using this for export pipeline markets (a region can only export to one pipeline)
             FILE = "energy/A21.stubtech_interp_R", #using this for import pipeline markets (a region can import from multiple pipelines)
             ...
             FILE = "energy/EIA_USA_energy_prices",
             "L202.CarbonCoef",
    ...
    GCAM_region_pipeline_bloc <- get_data(all_data, "energy/GCAM_region_pipeline_bloc", strip_attributes = TRUE)
    A21.stubtech_interp_R <- get_data(all_data, "energy/A21.stubtech_interp_R", strip_attributes = TRUE)
    ...
    EIA_USA_energy_prices <- get_data(all_data, "energy/EIA_USA_energy_prices", strip_attributes = TRUE)
    ...
    # determined by the mapping in A21.stubtech_interp_R
    ...
    A_ff_RegionalTechnology_NG %>%
      ...
      filter(year %in% MODEL_YEARS) %>%
      repeat_add_columns(GCAM_region_names) %>%
      full_join(A21.stubtech_interp_R, by = c("region", "supplysector", "subsector", "technology" ="stub.technology")) %>%
    ...
    L2392.SubsectorAll_tra_pipeline <- A_ff_TradedSubsector_NG %>%
      ...
      left_join_error_no_match(GCAM_region_pipeline_bloc, by = c("subsector_region" = "origin.region")) %>%
    ...
    A_ff_TradedTechnology_R_Y_pipeline <- repeat_add_columns(A_ff_TradedTechnology_NG,
      ...
      repeat_add_columns(GCAM_region_names) %>%
      left_join_error_no_match(GCAM_region_pipeline_bloc, by = c("region" = "origin.region")) %>%
      # keep only the rows for which the technology matches the region-specific pipeline
    ...
    A_ff_RegionalTechnology_R_Y_pipeline <- A_ff_RegionalTechnology_NG %>%
      ...
      # remove interpolation information, this will be written by regional-specific interp info from A21.stubtech_interp_R
      select(-apply.to, -from.year, -to.year, -to.value, -interpolation.function) %>%
      repeat_add_columns(tibble(year = MODEL_YEARS)) %>%
      repeat_add_columns(GCAM_region_names["region"]) %>%
      mutate(market.name = if_else(market.name == "regional", region, market.name)) %>%
      full_join(A21.stubtech_interp_R, by = c("region", "supplysector", "subsector", "technology" ="stub.technology")) %>%
    ...
    L2392.Production_reg_imp_pipeline <- A_ff_RegionalTechnology_R_Y_pipeline %>%
      ...
      # Correct subsector shareweights
    L2392.Production_reg_imp_pipeline_subs<-L2392.Production_reg_imp_pipeline %>%
      group_by(region, supplysector, subsector, year) %>%
      summarise(calOutputValue = sum(calOutputValue)) %>%
      ungroup() %>%
      rename(calOutputValue_subs = calOutputValue)

    L2392.Production_reg_imp_pipeline<-L2392.Production_reg_imp_pipeline %>%
      left_join_error_no_match(L2392.Production_reg_imp_pipeline_subs, by = c("region", "supplysector", "subsector", "year")) %>%
      mutate(subs.share.weight = if_else(calOutputValue_subs > 0, 1, 0)) %>%
      select(-calOutputValue_subs) %>%
      select(LEVEL2_DATA_NAMES[["Production"]], "subsector0")
    ...
    ## USA resource price adjustment, since we have data for USA
    EIA_USA_energy_prices %>%
      select(year = Year, price_hist = Natural_Gas) %>%
    ...
    L2392.SubsectorAll_tra_NG %>%
      ...
      add_precursors("common/GCAM_region_names",
                     ...
                     "energy/GCAM_region_pipeline_bloc") ->
      L2392.SubsectorAll_tra_NG
    ...
    L2392.gas_price_adj_USA %>%
      add_title("Regional historical natural gas resource price adjustment for USA") %>%
      add_units("1975$/GJ") %>%
      add_comments("Converted from EIA_USA_energy_prices") %>%
      add_precursors("energy/EIA_USA_energy_prices",
                     "L210.RsrcPrice") ->
      L2392.gas_price_adj_USA
