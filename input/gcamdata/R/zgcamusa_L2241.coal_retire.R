...
module_gcamusa_L2241.coal_retire <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    #BYU-TODO Update with newer EIA data that extends to the base-year
    #         Write warning if latest retirement year for coal plants in # File: EIA_860_generators_retired_2018.csv is > 1 year before latest base year
    ...
    #BYU-TODO These "constants" moved from constants.R because some programing needs to be done
    #BYU-TODO: The forumula below only works robustly for 5-year base-year increments. Need to:
    # 1) Generalize so that the last vintage group always includes the base-year
    # 2) We will want some min size for each vintage group
    
    COAL.MIN.VINAGE.SIZE <- 4 # Don't create any vintages that are < 4 years wide
    gcamusa.COAL_VINTAGE_BREAKS <- c(0, seq(1950, MODEL_FINAL_BASE_YEAR, 5))
    #BYU-TODO: Perhaps keep above formula for the first cut, but then either adjust last vintage 
    #          group or add new vintage group if needed

    #BYU-TODO: Change the below to be generated by code using gcamusa.COAL_VINTAGE_BREAKS instead of hard coding this by hand. 
