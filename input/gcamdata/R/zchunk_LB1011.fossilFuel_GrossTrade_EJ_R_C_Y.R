...
module_energy_LB1011.ff_GrossTrade <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    A21.stubtech_interp_R <- get_data(all_data, "energy/A21.stubtech_interp_R")
    GCAM_region_pipeline_bloc <- get_data(all_data, "energy/GCAM_region_pipeline_bloc")
    ...
    L1011.comtrade_ff_BiTrade_y_ctry_item_FULL %>%
      ...
      left_join_error_no_match(GCAM_region_pipeline_bloc, by = c("origin.region")) %>%
      ...
      semi_join(A21.stubtech_interp_R, by = c("destination.region" = "region", "pipeline.market")) %>%
