...
module_energy_batch_gas_trade_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L2392.gas_price_adj_USA <- get_data(all_data, "L2392.gas_price_adj_USA")
    ...
    create_xml("USA_NG_price.xml") %>%
      add_xml_data(L2392.gas_price_adj_USA, "RsrcPrice") %>%
      add_precursors("L2392.gas_price_adj_USA") ->
      USA_NG_price.xml
