 ...
 * \file res_input.cpp
 * \ingroup Objects
 * \brief The RESInput class source file.
...
// static initialize.
const string RESInput::XML_REPORTING_NAME = "input-res";
...
*/
const string& RESInput::getXMLNameStatic() {
    const static string XML_NAME = "res-input";
    return XML_NAME;
}
...
*/
const string& RESInput::getXMLReportingName() const{
    return XML_REPORTING_NAME;
}

const string& RESInput::getXMLName() const{
 ...
 */
RESInput::RESInput( const RESInput& aOther )
{
...
RESInput* RESInput::clone() const {
    return new RESInput( *this );
}

bool RESInput::isSameType( const string& aType ) const {
    return aType == getXMLNameStatic();
}

bool RESInput::XMLParse( rapidxml::xml_node<char>* & aNode ) {
...
void RESInput::toDebugXML( const int aPeriod,
                               ostream& aOut,
                               Tabs* aTabs ) const
{
...
void RESInput::completeInit( const string& aRegionName,
...
void RESInput::initCalc( const string& aRegionName,
                            const string& aSectorName,
 ...
 */
void RESInput::initializeTypeFlags() {
    
    // Set the flag to RES.
    mTypeFlags |= IInput::RES;
}

const string& RESInput::getMarketName( const string& aRegionName ) const {
    return mMarketName;
}

void RESInput::copyParam( const IInput* aInput,
                             const int aPeriod )
{
    aInput->copyParamsInto( *this, aPeriod );
}

void RESInput::copyParamsInto( RESInput& aInput,
...
double RESInput::getCO2EmissionsCoefficient( const string& aGHGName,
                                             const int aPeriod ) const
{
...
double RESInput::getPhysicalDemand( const int aPeriod ) const {
    assert( mPhysicalDemand[ aPeriod ].isInited() );
    return mPhysicalDemand[ aPeriod ];
}

double RESInput::getCarbonContent( const int aPeriod ) const {
    return mPhysicalDemand[ aPeriod ] * mCO2Coefficient;
}

void RESInput::setPhysicalDemand( double aPhysicalDemand,
...
double RESInput::getCoefficient( const int aPeriod ) const {
...
void RESInput::setCoefficient( const double aCoefficient,
                                  const int aPeriod )
{
...
double RESInput::getPrice( const string& aRegionName,
                              const int aPeriod ) const
{
...
void RESInput::setPrice( const string& aRegionName,
                            const double aPrice,
                            const int aPeriod )
{
...
double RESInput::getCalibrationQuantity( const int aPeriod ) const
{
    // return -1 if no calibration value is read in
    return mCalibrationInput.isInited() ? mCalibrationInput.get() : -1;
}

double RESInput::getIncomeElasticity( const int aPeriod ) const {
    return mIncomeElasticity;
}

double RESInput::getPriceElasticity( const int aPeriod ) const {
    return 0;
}

double RESInput::getTechChange( const int aPeriod ) const
{
    return mTechChange;
}

void RESInput::doInterpolations( const int aYear, const int aPreviousYear,
                                    const int aNextYear, const IInput* aPreviousInput,
                                    const IInput* aNextInput )
{
    const RESInput* prevEneInput = static_cast<const RESInput*>( aPreviousInput );
    const RESInput* nextEneInput = static_cast<const RESInput*>( aNextInput );
    
    /*!
     * \pre We are given a valid RESInput for the previous input.
     */
    assert( prevEneInput );
    
    /*!
     * \pre We are given a valid RESInput for the next input.
     */
    assert( nextEneInput );
